...
### Bundled third-party headers (`third_party/`)

Shipped with this tree — nothing to fetch:
...
`third_party/` (CUTLASS, cereal, NVTX, pybind11) ships with this tree, so there is
nothing to fetch:

```bash
cd StreamInfer
ls third_party/cutlass/include third_party/cereal/include \
   third_party/NVTX/c/include third_party/pybind11/include   # sanity check
