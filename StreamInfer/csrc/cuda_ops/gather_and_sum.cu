...
static GdrRing* gather_and_sum_src_ring = nullptr;
...
void gather_and_sum_tokens_cuda_dispatch(
    ...
    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    if (gather_and_sum_src_ring == nullptr)
        gather_and_sum_src_ring = new GdrRing(MAX_GATHER_TOKENS, torch::kUInt64);
    gdr_context_t gather_src_ptrs_gdr = gather_and_sum_src_ring->next(stream);
