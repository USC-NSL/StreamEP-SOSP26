...
__global__ void apply_weights_and_permute_tokens_kernel(T *d_out, T *d_in, float *d_weights, int *mappings, const int hidden_size) {
    ...
    // Two historical bugs fixed here: the apply_weights template flag was not
    // passed (defaulted to false, compiling the multiply OUT — expert outputs
    // were merged UNWEIGHTED), and the weight pointer was offset by token_id
    // and then indexed by token_id again (d_weights[2*token_id]).
    move_one_token_kernel<CHUNK_SIZE, true>(out, src, hidden_size, d_weights[token_id]);
}
...
// Event-fenced GDR ring for source pointers (replaces the racy 2-slot
// alternation that let the CPU lap the GPU under concurrency -> D20).
static GdrRing* gather_src_ring = nullptr;
...
void gather_tokens_cuda_dispatch(torch::Tensor dest, int64_t src_ptr, int64_t num_tokens, int64_t hidden_size) {
    ...
    cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    if (gather_src_ring == nullptr)
        gather_src_ring = new GdrRing(MAX_GATHER_TOKENS, torch::kUInt64);
    gdr_context_t gather_src_ptrs_gdr = gather_src_ring->next(stream);
