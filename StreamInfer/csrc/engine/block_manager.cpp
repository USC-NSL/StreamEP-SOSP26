...
void BlockManager::allocate(int req_id, int seq_len) {
    ...
    // seq_len == 0 registers an empty block list (no pre-existing context);
    // append_tokens allocates the first block when the first token arrives.
    int blocks_needed = seq_len > 0 ? (seq_len - 1) / block_size_ + 1 : 0;
...
std::vector<int> BlockManager::get_seq_block_ids(int req_id) {
...
void BlockManager::append_tokens(int req_id, int context_len, int num_tokens) {
    ...
    ASSERT (num_tokens >= 1);
    ASSERT (has_seq_block_list(req_id));
    // context_len == 0 is legal: a request entering with no pre-existing KV
    // (real-token admission at position 0); the first block is appended below.
...
void BlockManager::update_block_table(batch_metadata_t meta, const std::vector<int> &context_lens) {
    ...
    for (int i = 0; i < num_prefill_tokens; i++) {
        ...
        // Pre-admission KV ingestion (M-KVL) allocates the request's blocks
        // before the first scheduled batch; skip allocation in that case.
        if (has_seq_block_list(req_id)) {
            continue;
        }
