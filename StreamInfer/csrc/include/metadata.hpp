...
struct BatchMetadata {
    ...
    void duplicate_topk(int topk) {
        if (topk == 1) return;
        // TOKEN-MAJOR repeat ([t0,t0,..,t1,t1,..]) — must match the order of
        // exp_ids/topk_weights assigned from the model's [n, k] flatten and
        // the permute kernel's src = i / topk expansion. The previous tiling
        // scrambled per-copy metadata across requests for n > 1 batches.
        req_ids = repeat_vector(req_ids, topk);
        init_prefill_lens = repeat_vector(init_prefill_lens, topk);
        attn_dp_ranks = repeat_vector(attn_dp_ranks, topk);
        topk_weights = repeat_vector(topk_weights, topk);
        shape[0] *= topk;
    }
