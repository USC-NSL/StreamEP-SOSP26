...
struct TokenBatch: ScheduleUnit {
    ...
    inline static TokenBatch merge_by_expert(const std::vector<TokenBatch>& batches) {
        ...
        // The caller drops the source batches right after this returns, while
        // the gather below is only enqueued. Keep the allocator from reusing
        // the source blocks until this stream's pending work is done.
        for (auto &batch: batches) {
            batch.data.record_stream(at::cuda::getCurrentCUDAStream());
        }
    ...
    inline static TokenBatch pack_topk_tokens(int layer_id, std::vector<TokenTopKInfo>& tokens) {
        ...
        for (int i = 0; i < n; i++) {
            for (int k = 0; k < topk; k++) {
                ...
                tokens[i].topk_tensors[k].record_stream(at::cuda::getCurrentCUDAStream());
            }
        }
