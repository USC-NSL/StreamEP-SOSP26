...
// Element-repeat ([a,b] x2 -> [a,a,b,b]), matching the TOKEN-MAJOR order of
// the model's top-k outputs ([t0k0..t0k3, t1k0..]) and the permute kernel's
// src = i / topk expansion. duplicate_vector's tiling ([a,b,a,b]) misaligns
// every per-copy metadata field against exp_ids/hiddens whenever n > 1.
template<class T>
inline std::vector<T> repeat_vector(const std::vector<T> &a, int times) {
    std::vector<T> res;
    res.reserve(a.size() * times);
    for (const auto &x : a) {
        for (int i = 0; i < times; i++) {
