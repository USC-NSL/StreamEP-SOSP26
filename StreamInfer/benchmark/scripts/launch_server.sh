...
# real-model serving block (plan_jul18 §6.1; all independent, dummy mode = leave
# unset). ROUTER_MODE defaults to "random" HERE to preserve the historical dummy
# behavior of every existing caller of this script — the core default is "model"
# (F8); real-serving scripts set ROUTER_MODE=model explicitly.
MODEL_ARGS="$MODEL_ARGS --router-mode ${ROUTER_MODE:-random}"
if [ ! -z $WEIGHTS_DIR ]; then
    MODEL_ARGS="$MODEL_ARGS --weights-dir $WEIGHTS_DIR"
fi
if [ ! -z $TOKENIZER_PATH ]; then
    MODEL_ARGS="$MODEL_ARGS --tokenizer-path $TOKENIZER_PATH"
fi
if [ ! -z $SAMPLER ]; then
    MODEL_ARGS="$MODEL_ARGS --sampler $SAMPLER"
fi
if [ ! -z $KV_SOURCE ]; then
    MODEL_ARGS="$MODEL_ARGS --kv-source $KV_SOURCE"
fi
if [ ! -z $MOONCAKE_MASTER ]; then
    MODEL_ARGS="$MODEL_ARGS --mooncake-master $MOONCAKE_MASTER"
fi
