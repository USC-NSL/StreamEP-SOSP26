...
def override_model_config_with_args(args, model_config: ModelConfig):
    ...
    # real-model serving block (orthogonal flags; unset = dummy mode)
    model_config.weights_dir = getattr(args, "weights_dir", None)
    if getattr(args, "router_mode", None):
        model_config.router_mode = args.router_mode
    if getattr(args, "sampler", None):
        model_config.sampler = args.sampler
    if getattr(args, "kv_source", None):
        model_config.kv_source = args.kv_source
    model_config.mooncake_master = getattr(args, "mooncake_master", None)
    if getattr(args, "kv_model_tag", None):
        model_config.kv_model_tag = args.kv_model_tag
    return model_config
