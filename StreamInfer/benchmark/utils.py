...
def add_model_arguments(parser: ArgumentParser):
    ...
    # real-model serving block (plan_jul18 §6.1 — all orthogonal; dummy mode = leave unset)
    parser.add_argument("--weights-dir", type=str, default=None,
                        help="dir with per-rank shards + attn_shared (M-WL); unset = dummy random weights")
    parser.add_argument("--router-mode", type=str, default="model", choices=["model", "random"],
                        help="'model' = computed router logits (default); 'random' = rand_like override "
                             "(the historical dummy-experiment behavior — dummy scripts should pass this)")
    parser.add_argument("--tokenizer-path", type=str, default=None,
                        help="HF tokenizer path for real tokenization (M-TOK); unset = dummy tokenizer")
    parser.add_argument("--sampler", type=str, default="dummy", choices=["dummy", "model"],
                        help="sampler stack (M-SMP)")
    parser.add_argument("--kv-source", type=str, default="none", choices=["none", "mooncake"],
                        help="KV ingestion source (M-KVL)")
    parser.add_argument("--mooncake-master", type=str, default=None, help="mooncake master host:port")
    parser.add_argument("--kv-model-tag", type=str, default="gptoss120b", help="KV object key namespace tag")
