...
from disagmoe.models.utils import make_attention_dummy_batch, make_profile_decode_meta, make_expert_dummy_inputs
...
class AttnExecutor(Executor):
    ...
    def init_model(self):
        ...
        self.operators = [
            MoEAttention(
                ...
                sliding_window=self.model_config.sliding_window_of(
                    self.model_config.layer_ids[layer_id]),
                use_sinks=self.model_config.attention_sinks,
                attention_bias=self.model_config.attention_bias,
                router_norm=self.model_config.router_norm,
                router_bias=self.model_config.router_bias,
                router_mode=self.model_config.router_mode,
                first_layer=(self.model_config.layer_ids[layer_id] == 0),
        ...
        # M-WL: overwrite the randn init with real weights when configured;
        # runs before process_weights_after_loading so quantized layouts are
        # produced from the real values
        if self.model_config.weights_dir:
            from disagmoe.weights import load_attn_weights
            load_attn_weights(self.operators, self.model_config.layer_ids,
    ...
    def init_kv_cache(self, use_gpu_block_mgr: bool=False):
        ...
        # F1 residual stream: one stash row per running request holding
        # residual_l = x_l + attn_out_l between consecutive layers. Row
        # [max_running_reqs] is a scratch slot for padded graph rows.
        self.residual_stash = torch.zeros(
            (self.max_running_reqs + 1, self.model_config.hidden_size),
            dtype=self.model_config.dtype, device=self.device,
        )
        for operator in self.operators:
            operator.residual_stash = self.residual_stash
    ...
    def memory_profile(self, batch_size: int):
        # decode-shaped profiling batch (the direct flash_attn_with_kvcache path
        # needs a real paged cache; a one-block-per-token throwaway pool is
        # allocated per layer and freed immediately — a few MB, not part of the
        # steady-state footprint being measured)
        attn_metadata = make_profile_decode_meta(batch_size, self.device)
        for layer_id in range(self.num_layers):
            kv_cache = torch.zeros(
                (2, batch_size, self.cache_config.block_size,
    ...
    def execute_normal(self, batch: AttentionForwardBatch) -> AttentionForwardResult:
        outputs, topk_weights, topk_ids = self.execute_eager(batch.layer_id, batch.positions, batch.data, batch.metadata, request_ids=batch.req_ids, stash_slots=batch.stash_slots)
    ...
    def execute_graph(self, batch: AttentionForwardBatch) -> AttentionForwardResult:
        ...
        staging_outputs, staging_topk_weights, staging_topk_ids = self.cuda_graph_executor.run(batch.layer_id, batch.positions, batch.data, batch.metadata, request_ids=req_ids_tensor, stash_slots=batch.stash_slots)
...
class ExpertsExecutor(Executor):
    ...
    def __init__(self, model_config: ModelConfig, local_to_global_expert_rank: List[int], global_to_local_expert_rank: List[int]):
        ...
        for _ in range(self.num_layers):
            if self.expert_cls is MoEExpertsSerial:
                assert self.model_config.swiglu_limit is None and not self.model_config.expert_bias, \
                    "clamped activation / expert bias only implemented for the CUTLASS expert classes"
                ...
                if self.expert_cls in (MoEExpertsCUTLASS, MoEExpertsCUTLASSFP8):
                    extra_kwargs = dict(
                        swiglu_limit=self.model_config.swiglu_limit,
                        swiglu_alpha=self.model_config.swiglu_alpha,
                        with_bias=self.model_config.expert_bias,
                    )
                else:
                    assert self.model_config.swiglu_limit is None and not self.model_config.expert_bias, \
                        "clamped activation / expert bias only implemented for the CUTLASS expert classes"
        ...
        # M-WL: real expert weights for this worker's local experts
        if self.model_config.weights_dir:
            from disagmoe.weights import load_expert_weights
            load_expert_weights(self.operators, self.model_config.layer_ids,
    ...
    def prepare_bsz_and_indices(self, meta_c: BatchMetadata) -> Tuple[Optional[Union[Tensor, List[int]]], Optional[Tensor]]:
        ...
        # DeepGEMM-based experts expect m_indices; the CUTLASS classes need them
        # too when per-expert biases are enabled (F7: token->local-expert gather)
        needs_m_indices = self.expert_cls in [MoEExpertsDeepGemmBF16, MoEExpertsDeepGemmFP8] or (
            self.expert_cls in (MoEExpertsCUTLASS, MoEExpertsCUTLASSFP8)
            and self.model_config.expert_bias
        )
        if needs_m_indices:
    ...
    def execute_eager(self, batch: ExpertForwardBatch) -> Tensor:
        ...
        # Dispatch: CUTLASS (bf16/fp8) uses batch_sizes (+ m_indices for biases);
        # DeepGEMM uses m_indices
        if self.expert_cls in (MoEExpertsCUTLASS, MoEExpertsCUTLASSFP8):
            outputs = self.operators[vid].forward(batch.num_tokens, batch.data, batch.batch_sizes, m_indices=batch.m_indices)
