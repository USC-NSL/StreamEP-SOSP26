...
class CUDAGraphAttnExecutor:
    ...
    def create_cuda_graph_buffers(self):
        ...
        # F1: residual-stash row per token. Default-filled with the scratch row
        # (stash's last row) so padded graph rows read/write scratch harmlessly;
        # run() overwrites [:num_tokens] with the batch's real slots. Stash
        # writes before a slot's first layer-0 write are garbage but dead: every
        # request's stream begins with a layer-0 stash WRITE before any read.
        self.stash_scratch_slot = self.attn_executor.max_running_reqs
        self.static_stash_slots = torch.full(
            (batch_size,), self.stash_scratch_slot, dtype=torch.int64, device="cuda")

        # F5: the in-kernel KV append (flash_attn_with_kvcache with k=/v=)
        # writes EVERY row, padded ones included — unlike the old
        # reshape_and_cache path, which skipped slot_mapping == -1. The fused
        # preprocess zeroes padded context_lens but leaves padded block-table
        # rows STALE, so a padded row would append its garbage K/V into offset
        # 0 of a real request's first block. Redirect padded rows to the KV
        # pool's never-allocated spare block (allocator hands out
        # [0, num_blocks); the pool has num_blocks+1 page rows).
        self.kv_scratch_block = self.attn_executor.num_cache_blocks
            ...
            for layer_id in self.model_config.layer_ids:
                ...
                def run_once() -> Tuple[Tensor, Tensor, Tensor]:
                    ...
                    return self.attn_executor.execute_eager(
                        ...
                        request_ids=req_ids,
                        stash_slots=self.static_stash_slots[ : graph_batch_size],
                    )
    ...
    def run(self, layer_id: int, positions: torch.Tensor, hidden_states: torch.Tensor, meta: FlashAttentionMetadata, request_ids: torch.Tensor = None, stash_slots: torch.Tensor = None) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        try:
            ...
            if stash_slots is not None:
                self.static_stash_slots[ : num_tokens].copy_(stash_slots)
                if num_tokens < batch_size:
                    self.static_stash_slots[num_tokens : batch_size].fill_(self.stash_scratch_slot)
            if num_tokens < batch_size:
                # padded rows: context 0 + scratch block => harmless KV append
                self.static_block_table[num_tokens : batch_size, 0].fill_(self.kv_scratch_block)
...
class CUDAGraphExpertsExecutor:
    ...
    def cuda_graph_preprocess(self, hidden_states: torch.Tensor, batch_sizes: torch.Tensor, m_indices: torch.Tensor, bucket_size: int):
        ...
        if self.expert_cls in (MoEExpertsCUTLASS, MoEExpertsCUTLASSFP8):
            ...
            # Simple D2D copies; m_indices only when expert biases need the
            # per-token gather (F7).
            self.static_input_hiddens[:num_tokens].copy_(hidden_states)
            self.static_input_batch_sizes.copy_(batch_sizes)
            if m_indices is not None and getattr(self.experts_executor.operators[0], "with_bias", False):
                self.static_input_m_indices[:num_tokens].copy_(m_indices)
        else:
