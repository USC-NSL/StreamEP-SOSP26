...
class AttentionEngineMixin:
    ...
    def build_attn_executor(self):
        ...
        if use_gdrcopy_optimization:
            ...
            self.stash_slots_gdr = GdrDoubleBuffer(self.engine_config.max_batch_size_attn, dtype=torch.int64, device="cuda")
        else:
            ...
            self.stash_slots_gdr = None

        # F1 residual-stash slot pool: one row per running request, assigned on
        # first sight (a request's first batch on this worker is its layer-0
        # entry) and returned on release
        self.stash_free_slots: Deque[int] = deque(range(self.attn_executor.max_running_reqs))
        self.stash_slot_map: Dict[int, int] = {}
    ...
    def preprocess_batch_attn(self, batch: TokenBatchCWrapper) -> Optional[AttentionForwardBatch]:
        ...
        # F2: the token being decoded sits at index seq_len-1 = context_len
        # (seq_lens already counts it); RoPE must see its true position.
        # MUST slice to the batch: the GDR-rebind metadata views span the whole
        # max-batch buffer, and the vllm rope kernel launches over
        # positions.numel() tokens — an unsliced view makes it read/write far
        # past the 1..n-row q/k tensors (latent OOB present in the original
        # positions=seq_lens code as well).
        num_tokens = schedule_batch.num_tokens()
        positions = attn_meta.context_lens_tensor[:num_tokens].to(torch.int64)

        import os as _os
        if _os.environ.get("DMOE_LAYER_DUMP", ""):
            import disagmoe.models.attention as _attn_mod
            _attn_mod._DBG_REQ_IDS = list(schedule_batch.req_ids)

        # F1: per-token residual-stash rows for this batch
        stash_slots_list = [self._stash_slot_of(rid) for rid in schedule_batch.req_ids]
        if len(stash_slots_list) == 0:
            stash_slots = None
        elif use_gdrcopy_optimization:
            slots_gdr = self.stash_slots_gdr.get_one_handle()
            slots_gdr.copy_from_host_int64(stash_slots_list)
            stash_slots = slots_gdr.tensor[: len(stash_slots_list)]
        else:
            stash_slots = torch.tensor(stash_slots_list, dtype=torch.int64, device="cuda")
    ...
    def sample_results(self, batch: AttentionScheduleBatch) -> TokenBatchCWrapper:
        # F1: the batch data arriving at the final pseudo-layer is the bare
        # last-layer expert sum; adding the stash row completes the residual
        # stream. The completed stream is what the sampler consumes AND what
        # continuing requests loop back (dummy mode included — fidelity).
        stash_slots_list = [self._stash_slot_of(rid) for rid in batch.req_ids]
        if len(stash_slots_list) > 0 and use_gdrcopy_optimization:
            slots_gdr = self.stash_slots_gdr.get_one_handle()
            slots_gdr.copy_from_host_int64(stash_slots_list)
            slots_tensor = slots_gdr.tensor[: len(stash_slots_list)]
        else:
            slots_tensor = torch.tensor(stash_slots_list, dtype=torch.int64, device="cuda")
        completed = batch.data + self.attn_executor.residual_stash.index_select(0, slots_tensor)

        continue_ids, finish_req_ids, out_token_ids, continued_data = \
            self.dummy_sampler.step(completed, batch.req_ids)

        _dump_dir = ENV_VARS.get("DMOE_ORACLE_DUMP", "") if isinstance(ENV_VARS, dict) else ""
        if not _dump_dir:
            import os as _os
            _dump_dir = _os.environ.get("DMOE_ORACLE_DUMP", "")
        if _dump_dir:
            import os as _os
            _os.makedirs(_dump_dir, exist_ok=True)
            self._oracle_step = getattr(self, "_oracle_step", 0)
            torch.save({"req_ids": list(batch.req_ids),
                        "completed": completed.detach().cpu(),
                        "data": batch.data.detach().cpu(),
                        "out_tokens": list(out_token_ids)},
                       f"{_dump_dir}/step{self._oracle_step:04d}_dev{self.device_id}.pt")
            self._oracle_step += 1
        ...
        if continued_data is None:
            # dummy sampler: loop the completed stream back as-is
            if len(continue_ids) == 0:
                continued_data = completed[:0]
            elif use_gdrcopy_optimization:
                ...
                continued_data = torch.index_select(completed, 0, idx_tensor)
            else:
                continued_data = completed[continue_ids]
    ...
    def _stash_slot_of(self, req_id: int) -> int:
        slot = self.stash_slot_map.get(req_id)
        if slot is None:
            slot = self.stash_free_slots.popleft()
            self.stash_slot_map[req_id] = slot
        return slot

    def release_seqs(self, seq_ids: List[int]):
        self.block_mgr.release_seqs(seq_ids)
        for rid in seq_ids:
            slot = self.stash_slot_map.pop(rid, None)
            if slot is not None:
                self.stash_free_slots.append(slot)
...
class ExpertEngineMixin:
    ...
    def execute_batch_expert(self, batch: ExpertForwardBatch) -> ExpertForwardResult:
        ...
        import os as _os
        _edbg = _os.environ.get("DMOE_EXPERT_DUMP", "")
        if _edbg:
            _os.makedirs(_edbg, exist_ok=True)
            self._edbg_step = getattr(self, "_edbg_step", 0)
            torch.save({
                "meta_weights": list(topk_weights) if not torch.is_tensor(topk_weights) else topk_weights.detach().cpu(),
                "weights_tensor_head": expert_weights_tensor[:16].detach().cpu(),
                "mappings_head": expert_token_mapping_tensor[:16].detach().cpu(),
                "exp_ids": list(batch.meta_c.exp_ids),
                "hiddens_in": hiddens.detach().cpu(),
            }, f"{_edbg}/exp_step{self._edbg_step:04d}.pt")

        permuted_tokens = apply_weights_and_permute_tokens(hiddens, expert_weights_tensor, expert_token_mapping_tensor)

        if _edbg:
            torch.save(permuted_tokens.detach().cpu(), f"{_edbg}/exp_step{self._edbg_step:04d}_out.pt")
            self._edbg_step += 1
    ...
    def process_batch_expert(self, batch: TokenBatchCWrapper) -> Optional[TokenBatchCWrapper]:
        ...
        try:
            ...
            with open(f"/tmp/engine-expert-{self.device_id}.err", "wt") as f:
...
class Engine(AttentionEngineMixin, ExpertEngineMixin, EngineProfilerMixin):
    ...
    def init_core(self, core_args: InitCoreArgs):
        ...
        if self.has_attn:
            # M-SMP: sampler stack is config-selected; both classes implement
            # the same create_request/admit_data/step/clean_request interface
            if getattr(self.model_config, "sampler", "dummy") == "model":
                from disagmoe.sampler import RealSampler
                self.dummy_sampler = RealSampler(
                    self.model_config.weights_dir,
                    self.model_config.eos_token_ids,
                    self.model_config.rms_norm_eps,
                    device="cuda",
                    dtype=self.model_config.dtype,
                    kv_source=getattr(self.model_config, "kv_source", "none"),
                )
            else:
        ...
        # M-KVL: mooncake KV ingestion (config-driven; None in dummy / kv-none)
        self.kv_loader = None
        if self.has_attn and getattr(self.model_config, "kv_source", "none") == "mooncake":
            import os as _os
            from disagmoe.kv_loader import MooncakeKVLoader
            from disagmoe.utils.utils import get_ip
            local_ip = _os.environ.get("STREAMEP_KV_LOCAL_IP", "") or get_ip()
            self.kv_loader = MooncakeKVLoader(
                self.model_config.mooncake_master, local_ip,
                self.attn_executor.kv_cache,
                self.block_mgr._block_mgr,
                self.cache_config.block_size,
                len(self.model_config.layer_ids),
    ...
    def recv_new_request(self):
        try:
            ...
            # With a KV store, the prompt KV must be resident before the
            # request may schedule: hand the fetch to the async loader and
            # admit on completion (_drain_kv_loads). token_ids is the prompt
            # plus the first generated token from the prefill pass.
            if self.kv_loader is not None:
                self.kv_loader.submit(new_request.req_id,
                                      new_request.token_ids[:-1], new_request)
                return
            self._admit_request(new_request)
        except zmq.Again:
            pass

    def _admit_request(self, new_request: "TokenizedRequest"):
        self.record_max_output_lens(new_request.req_id, new_request.max_output_len,
                                    new_request.token_ids)
        data, prefill_len = self.dummy_sampler.admit_data(
            new_request.req_id, new_request.init_prefill_len, new_request.token_ids)
        meta = BatchMetadata(
            ...
            init_prefill_lens=[prefill_len],
        )
        batch: TokenBatch = TokenBatch_C()
        if data is None:
            data = torch.rand((1, self.model_config.hidden_size), dtype=torch.bfloat16, device=self.device)
        batch.data = data
        batch.metadata = meta.to_c()
        self.pool.put_batch(batch)
        self._advanced_logger.log_pool_put(1)

    def _drain_kv_loads(self):
        """Install completed KV fetches and admit their requests. A request
        whose KV cannot be fetched is rejected (finished result with no
        tokens) so the engine keeps serving — no prefill fallback exists."""
        if self.kv_loader is None:
            return
        for done in self.kv_loader.poll():
            req: TokenizedRequest = done.job.ctx
            if self.kv_loader.install(done) and \
                    done.n_pages * 16 == req.init_prefill_len:
                self._admit_request(req)
            else:
                get_logger().error(
                    f"KV load failed for req {req.req_id}; rejected")
                self.detokenizer_socket.send_pyobj(BatchDecodeResult(
                    req_ids=[req.req_id],
                    token_ids=[0],
                    is_eos=[True]))
    ...
    def single_module_loop_overlap(self):
        ...
        try:
            ...
            with open(f"/tmp/engine-{self.device_id}.err", "wt") as f:
    ...
    def record_max_output_lens(self, req_id: int, max_output_len: int, token_ids=None):
        assert self.has_attn
        self.dummy_sampler.create_request(req_id, max_output_len, token_ids=token_ids)
...
class DummySampler:
    ...
    # --- unified sampler interface (shared with disagmoe.sampler.RealSampler) ---
    def step(self, completed_hidden: Tensor, req_ids: List[int]):
        continue_ids, finish_req_ids = self.sample_once(req_ids)
        # None continued_data: engine loops completed_hidden[continue_ids] back
        return continue_ids, finish_req_ids, [0] * len(req_ids), None

    def admit_data(self, req_id: int, init_prefill_len: int, token_ids):
        # None data: engine keeps its existing rand admission tensor
        return None, init_prefill_len

    def create_request(self, req_id: int, max_output_len: int, token_ids=None):
