...
class Detokenizer:
    ...
    def __init__(self):
        ...
        # M-TOK: per-request emitted token ids, in emission order (dummy mode
        # emits zeros — harmless); fetched by the driver for decoding/oracles
    ...
    def process_batch(self, batch: BatchDecodeResult):
        ...
        for i in range(num_tokens):
            rid = batch.req_ids[i]
            self.token_streams.setdefault(rid, []).append(batch.token_ids[i])
    ...
    def fetch_token_streams(self) -> Dict[int, List[int]]:
        with self.lock:
            return {rid: list(toks) for rid, toks in self.token_streams.items()}
...
class Tokenizer:
    ...
    def __init__(self, attn_dp_size: int):
        ...
        # M-REQ: real prompt token ids registered ahead of dispatch (keyed by
        # req_id); absent = dummy request with the historical [0] placeholder
        self.pending_token_ids: Dict[int, List[int]] = {}

    def register_prompt(self, req_id: int, token_ids: List[int]) -> None:
        self.pending_token_ids[req_id] = list(token_ids)
    ...
    def put_single_request(
        ...
        """
        token_ids = self.pending_token_ids.pop(req_id, [0])
        tokenized_req = TokenizedRequest(req_id, init_prefill_len, max_output_len, token_ids)
