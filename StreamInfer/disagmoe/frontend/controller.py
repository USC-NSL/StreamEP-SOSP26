...
class Controller:
    ...
    def put_single_request(self, input_len: int, output_len: int,
                           token_ids: Optional[List[int]] = None) -> AsyncResult:
        dp_scheduler = self.dp_scheduler
        assert dp_scheduler is not None
        ...
        if token_ids is not None:
            # M-REQ: register the real prompt before dispatch so the tokenizer
            # actor attaches it to the TokenizedRequest
            ray.get(self.tokenizer.register_prompt.remote(req_id, token_ids))
    ...
    def fetch_token_streams(self) -> Dict[int, List[int]]:
        return ray.get(self.detokenizer.fetch_token_streams.remote())
