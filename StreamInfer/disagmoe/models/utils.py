...
def make_profile_decode_meta(batch_size: int, device: str = "cuda") -> FlashAttentionMetadata:
    """Decode-shaped metadata for memory profiling: each token owns one page
    (block_tables[i] = [i]) with zero context, so the in-kernel KV append of the
    direct flash_attn_with_kvcache path writes disjoint, valid slots."""
    return FlashAttentionMetadata(
        ...
        seq_lens_tensor=torch.ones(batch_size, dtype=torch.int32, device=device),
        max_query_len=0,
        max_prefill_seq_len=0,
        max_decode_seq_len=1,
        max_decode_query_len=1,
        query_start_loc=torch.arange(batch_size + 1, dtype=torch.int32, device=device),
        seq_start_loc=torch.arange(batch_size + 1, dtype=torch.int32, device=device),
        context_lens_tensor=torch.zeros(batch_size, dtype=torch.int32, device=device),
        block_tables=torch.arange(batch_size, dtype=torch.int32, device=device).view(-1, 1),
        use_cuda_graph=False,
        multi_modal_placeholder_index_maps=None,
        enable_kv_scales_calculation=True,
    )
