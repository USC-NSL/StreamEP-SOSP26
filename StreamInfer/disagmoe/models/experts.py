...
def _apply_glu_activation(
    cache_up: torch.Tensor,
    intermediate_size: int,
    act_fn,
    swiglu_limit,
    swiglu_alpha: float,
) -> torch.Tensor:
    """Config-selected gated activation on the fused [gate | up] projection.

    swiglu_limit None: standard SwiGLU, SiLU(gate) * up.
    swiglu_limit set (gpt-oss): gate/up clamped, (up + 1) * gate * sigmoid(alpha * gate)
    (matches HF transformers GptOssExperts._apply_gate with de-interleaved halves).
    """
    gate = cache_up[:, :intermediate_size]
    up = cache_up[:, intermediate_size:]
...
class MoEExpertsCUTLASS(torch.nn.Module):
    ...
    def _create_bias_weights(self, params_dtype: torch.dtype):
        if not self.with_bias:
            self.w13_bias = None
            self.w2_bias = None
            return
        self.w13_bias = torch.nn.Parameter(
            torch.randn(self.num_experts, self.intermediate_size * 2,
                        dtype=params_dtype).cuda(),
            requires_grad=False,
        )
        self.register_parameter("w13_bias", self.w13_bias)
    ...
    def create_weights(self, params_dtype: torch.dtype):
        ...
        self._create_bias_weights(params_dtype)
    ...
    def forward(self, bs: int, hiddens: torch.Tensor, batch_sizes: torch.Tensor,
        ...
        if self.w13_bias is not None:
            assert m_indices is not None, "expert bias needs token->local-expert m_indices"
            cache_up = cache_up + self.w13_bias[m_indices.to(torch.long)]
        up = _apply_glu_activation(cache_up, self.intermediate_size, self.act_fn,
                                   self.swiglu_limit, self.swiglu_alpha)
        ...
        if self.w2_bias is not None:
            down_out = down_out + self.w2_bias[m_indices.to(torch.long)]
        return down_out
...
class MoEExpertsCUTLASSFP8(torch.nn.Module):
    ...
    def create_weights(self):
        ...
        # biases stay bf16 regardless of fp8 weights
        if self.with_bias:
            self.w13_bias = torch.nn.Parameter(
                torch.randn(E, N13, device="cuda", dtype=torch.bfloat16),
                requires_grad=False,
            )
            self.w2_bias = torch.nn.Parameter(
                torch.randn(E, N2, device="cuda", dtype=torch.bfloat16),
                requires_grad=False,
            )
        else:
            self.w13_bias = None
