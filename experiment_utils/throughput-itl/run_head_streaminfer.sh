...
# sharegpt gate profile via --gate-profile-file), sweeps $RATES via /run_once,
# parses results.
set -uo pipefail
...
log "StreamInfer sweep | model=$MODEL layers=$NUM_LAYERS world=$WORLD_SIZE gate_profile=$(basename "$GATE_PROFILE") | workers=[$WORKER_HOSTS] rates=[$RATES] time=${BENCH_TIME}s"
start_ray_head
launch_workers
...
for battempt in $(seq 1 "$BOOT_RETRIES"); do
  log "Launching StreamInfer server (once, reused for all rates; boot attempt ${battempt}/${BOOT_RETRIES})..."
...
[ "$booted" -eq 1 ] || { tail -30 "$server_log"; kill_server; "$RAY_BIN" stop --force >/dev/null 2>&1; exit 1; }
