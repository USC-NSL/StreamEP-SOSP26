...
set_ucx_env() {
    if [ "$UCX_TRANSPORT" = "tcp" ]; then
        ...
        # rc = RDMA data path (needed so interference fair-shares with NCCL's RDMA on RoCE).
        # SPHERE quirks (all overridable via env — see interference-resist/README):
        #  - UCX_LIB_DIR: pin UCX>=1.18 (1.16's tcp_sockcm listener is broken); ~/ucx118 bundle.
        #  - device list: mlx5_1 on sgpu6/7/8, rocep225s0f1 on sgpu9 (absent = harmless WARN),
        #    plus a netdev (ens1f1np1) so the tcp connection-manager can do the wireup.
        #  - SOCKADDR_TLS_PRIORITY=tcp,...: use tcp_sockcm for wireup — RoCE rdma_cm rejects
        #    UCX's endpoint address (private-data too small). Bulk data still rides rc (RDMA).
        UCX_ENV="LD_LIBRARY_PATH=${UCX_LIB_DIR:-$HOME/ucx118/lib} UCX_TLS=${UCX_RC_TLS:-rc,tcp} UCX_NET_DEVICES=${UCX_NET_DEV_LIST:-mlx5_1:1,rocep225s0f1:1,ens1f1np1} UCX_IB_GID_INDEX=${UCX_IB_GID_INDEX:-3} UCX_SOCKADDR_TLS_PRIORITY=${UCX_SOCKADDR_TLS_PRIORITY:-tcp,sockcm,rdmacm}"
    fi
}
set_ucx_env
...
COUNTER_MULT=1
if [ "$UCX_TRANSPORT" = "tcp" ]; then
    # TCP rides the kernel netdev — read its byte counters.
...
read_counters() {
    # Read TX and RX byte counters from a host, print "tx rx" on one line.
    local host="$1"
    if [ "$UCX_TRANSPORT" = "tcp" ]; then
        run_on "$host" "echo \$(cat $COUNTER_TX 2>/dev/null || echo 0) \$(cat $COUNTER_RX 2>/dev/null || echo 0)" 2>/dev/null
    else
        # rc/RDMA bypasses the netdev stack — read the IB port counters instead
        # (port_xmit_data/port_rcv_data are in 4-byte words). The datapath device is
        # named mlx5_1 on most nodes but rocep225s0f1 on sgpu9, so try those in order.
        run_on "$host" 'for dev in mlx5_1 rocep225s0f1; do c=/sys/class/infiniband/$dev/ports/1/counters; [ -e "$c/port_xmit_data" ] || continue; echo $(( $(cat "$c/port_xmit_data")*4 )) $(( $(cat "$c/port_rcv_data")*4 )); break; done' 2>/dev/null
    fi
}
