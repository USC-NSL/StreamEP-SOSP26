...
#define MAX_OUTSTANDING     16            /* default max in-flight sends */
...
#define TOKEN_BUCKET_BURST  (1024*1024)  /* 1 MB default max burst     */
...
/* Contention pipeline depth (runtime-tunable). The defaults (16 x 64KB =
 * 1 MB in flight, 1 MB burst) pace beautifully on an idle link but collapse
 * under congestion: once queueing inflates RTT to ~1 ms, throughput caps at
 * in_flight/RTT (~8 Gbps/stream) no matter what the schedule asks, and the
 * token bucket silently discards the shortfall — the generator yields
 * instead of contending. Raise these (e.g. 64 / 8 MB) so achieved ~= offered
 * when the link is shared with a competing RDMA workload. */
static int    g_max_outstanding = MAX_OUTSTANDING;
static double g_burst_bytes     = (double)TOKEN_BUCKET_BURST;
...
static void tb_init(token_bucket_t *tb, double rate_bps) {
    tb->tokens      = g_burst_bytes;
    tb->rate_bps    = rate_bps;
    tb->last_ns     = now_ns();
    tb->burst_limit = g_burst_bytes;
}
...
static void run_rate_loop(schedule_t *sched, ucp_ep_h ep,
    ...
    for (uint64_t w = 0; w < total_windows && !g_stop; w++) {
        ...
        while (now_ns() < window_end && !g_stop) {
            /* Pipeline management */
            while (g_outstanding >= g_max_outstanding && !g_stop)
                ucp_worker_progress(worker);
            if (g_stop) break;
...
static void usage(const char *prog) {
    fprintf(stderr,
        ...
        "  --max-outstanding <N>  Max in-flight sends (default: %d)\n"
        "  --burst-bytes <N>   Token-bucket burst cap (default: %d)\n",
        prog, DEFAULT_MSG_SIZE, (unsigned long long)DEFAULT_WINDOW_NS,
        MAX_OUTSTANDING, TOKEN_BUCKET_BURST);
}
...
int main(int argc, char **argv) {
    ...
    setvbuf(stdout, NULL, _IOLBF, 0);   /* line-buffer stats even when logged to a file */
    ...
    static struct option long_opts[] = {
        ...
        {"max-outstanding", required_argument, 0, 'o'},
        {"burst-bytes",     required_argument, 0, 'b'},
    ...
    while ((c = getopt_long(argc, argv, "", long_opts, NULL)) != -1) {
        switch (c) {
            ...
            case 'o': g_max_outstanding = atoi(optarg); break;
            case 'b': g_burst_bytes = strtod(optarg, NULL); break;
            case 'h': usage(argv[0]); return 0;
            default:  usage(argv[0]); return 1;
        }
    }
    if (g_max_outstanding < 1) g_max_outstanding = 1;
    if (g_burst_bytes < (double)msg_size) g_burst_bytes = (double)msg_size;
