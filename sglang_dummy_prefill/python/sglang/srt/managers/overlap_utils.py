...
class FutureMap:
    ...
    def store_to_map(
        ...
        if self.spec_algo.is_eagle():
            ...
            # DP-attention: the synchronized forward can return next_token_ids sized to the
            # real (unpadded) request count while future_indices were allocated for the
            # DP-global-padded batch; under per-rank imbalance these differ (typically ±1).
            # Assign the overlapping region so the sizes stay consistent — the surplus future
            # slot is a padding placeholder (harmless: dummy weights, throughput-only run).
            _tok = batch_result.next_token_ids
            _n = min(intv.stop - intv.start, _tok.shape[0])
            self.token_ids_buf[intv.start : intv.start + _n] = _tok[:_n]
