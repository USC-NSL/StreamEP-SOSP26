...
class ServerArgs:
    ...
    def __post_init__(self):
        ...
        # Route --num-hidden-layers-override through the config-override machinery so
        # it actually shrinks the model for every architecture (some models, e.g.
        # gpt-oss, otherwise ignore it and load the full depth). Editing the HF config
        # this way keeps weight allocation, KV-cache sizing, and the model consistent.
        if self.num_hidden_layers_override is not None:
            _ov = json.loads(self.json_model_override_args or "{}")
            _ov.setdefault("num_hidden_layers", self.num_hidden_layers_override)
            self.json_model_override_args = json.dumps(_ov)
