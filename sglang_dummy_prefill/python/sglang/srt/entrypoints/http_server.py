...
async def start_profile_async(obj: Optional[ProfileReqInput] = None):
    ...
    await _global_state.tokenizer_manager.start_profile(
        ...
        start_min_batch_size=obj.start_min_batch_size,
...
def launch_server(
    ...
    try:
        ...
        # TCP accept-queue depth for the HTTP frontend. uvicorn's default is 2048,
        # which a benchmark client that opens one connection per request overruns
        # under overload: with >2048 connections pending accept, the kernel drops the
        # handshake's final ACK (net.ipv4.tcp_abort_on_overflow=0), the client believes
        # it is ESTABLISHED and POSTs, and the server kernel answers RST -- surfacing as
        # client-side ECONNRESET/connect-timeout "failed requests" that never reached the
        # scheduler at all. Raise this (and net.core.somaxconn, which caps it) so overload
        # shows up as queueing latency instead of connection errors.
        _http_backlog = int(os.environ.get("SGLANG_HTTP_BACKLOG", "2048"))
