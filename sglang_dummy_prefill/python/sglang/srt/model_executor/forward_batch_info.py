...
class ForwardBatch:
    ...
    def _pad_tensor_to_size(self, tensor: torch.Tensor, size: int, *, value: int = 0):
        pad = size - tensor.shape[0]
        # DP-attention: the all-gathered global max can lag this rank's actual batch by
        # ±1 under overlap + fake-prefill, making `size` smaller than the tensor. Clamp
        # (truncate) instead of creating a negative-dim tensor and crashing — the padded
        # slot is a placeholder and token values are moot on dummy weights / throughput runs.
        if pad <= 0:
            return tensor[:size]
        if value == 0:
            return torch.cat(
                [tensor, tensor.new_zeros(pad, *tensor.shape[1:])],
                dim=0,
            )
        else:
            return torch.cat(
                [
                    tensor,
                    tensor.new_full((pad, *tensor.shape[1:]), value),
                ],
                dim=0,
            )
